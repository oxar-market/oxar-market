...
export type Stage = {
  ...
  /**
   * Показывать ли места на вещи.
   *
   * Отдельно от `look`, потому что это разные вопросы: чем показана вещь и
   * что на ней сейчас продаётся. До открытия торга мест нет ни на ткани, ни
   * на голограмме - обводить то, чего ещё не продают, значит обещать лишнее.
   * После открытия они есть в любом режиме.
   */
  spots(show: boolean): void;
  /**
    ...
    (async () => {
      try {
        ...
        for (const spot of SPOTS) {
          ...
          // Скрыто до первого `spots(true)`: пока лоты не доехали, про торг
          // ничего не известно, и показывать места рано.
          mesh.visible = false;
          scene.add(mesh);
        ...
        stage.current = {
          ...
          look(mode) {
            ...
            // Места голограмма больше не прячет. Раньше прятала, потому что
            // показывалась только до открытия торга - обводить то, чего ещё не
            // продают, значило обещать лишнее. Теперь это обычный режим
            // просмотра, и выбирать места в нём надо так же, как на ткани.
            // Чем показывать вещь и что на ней продаётся - разные вопросы:
            // видимостью мест распоряжается `spots`, а не этот переключатель.
            if (shadow) shadow.visible = !ghosting;
          },
          spots(show) {
            for (const decal of decals) {
              decal.mesh.visible = show;
              // Рамку прячем вместе с местами; показываем - возвращает
              // restyle, и только выбранному.
              if (!show) decal.frame.visible = false;
            }
            if (show) restyle();
          },
          face(azimuth) {
