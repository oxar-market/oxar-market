...
pub struct Deal {
    ...
    ///
    /// Одно поле вместо трёх режимов. Там, где размещение проверяется само -
    /// профиль в сети, - ставим конец срока: покупатель волен отменить в любой
    /// момент, и это и есть обещание «платим за простоявшее время». Там, где
    /// проверить нечем, а продавец несёт невозвратные расходы до начала -
    /// печать, изготовление, бронь площадки, - ставим момент, с которого эти
    /// расходы начались. После него деньги заперты для обеих сторон: покупатель
    /// не вернёт, продавец не возьмёт, пока не наступит `ends_at`.
    ///
    /// Слова «печать» здесь нет намеренно. Программа не знает, что продают: она
    /// знает только, с какой секунды отказ перестаёт быть бесплатным.
    pub refundable_until: i64,
...
impl Deal {
    ...
    ///
    /// Два окна, между ними заморозка. До `refundable_until` он волен
    /// передумать. После `ends_at` закрывать уже нечего - вся сумма заработана,
    /// и закрытие лишь возвращает ренту, так что запрещать его незачем.
    /// Посередине деньги не принадлежат ни одной стороне окончательно, и
    /// трогать их нельзя никому.
    pub fn buyer_may_close(&self, now: i64) -> bool {
        now < self.refundable_until || now >= self.ends_at
    }
}
...
mod tests {
    ...
    #[test]
    fn поток_покупатель_может_отменить_в_любой_момент() {
        // Место в профиле: проверяется само, и право отмены - это и есть
        // обещание продукта. Окно отказа равно всему сроку.
        let d = deal(1_000, 4_600, 100);
        assert!(d.buyer_may_close(999), "до начала нельзя отменить");
        assert!(d.buyer_may_close(1_000), "на старте нельзя отменить");
        assert!(d.buyer_may_close(2_800), "в середине нельзя отменить");
        assert!(d.buyer_may_close(9_999), "после конца нельзя закрыть");
    }

    #[test]
    fn заморозка_покупатель_не_достанет_деньги_из_середины() {
        // Футболка: срок нулевой и стоит в конце события, а расходы у продавца
        // начинаются раньше. Между этими моментами деньги не принадлежат никому.
        let mut d = deal(8_000, 8_000, 100);
        d.refundable_until = 1_000;

        assert!(d.buyer_may_close(999), "до расходов отказ должен быть свободен");
        assert!(!d.buyer_may_close(1_000), "отказ в момент начала расходов прошёл");
        assert!(!d.buyer_may_close(5_000), "покупатель достал деньги из заморозки");
        assert!(!d.buyer_may_close(7_999), "отказ за секунду до конца прошёл");
        assert!(d.buyer_may_close(8_000), "после конца закрыть нельзя");
    }

    #[test]
    fn в_заморозке_продавцу_тоже_ничего_не_натекает() {
        let mut d = deal(8_000, 8_000, 100);
        d.refundable_until = 1_000;
        assert_eq!(d.earned_at(5_000).unwrap(), 0, "продавец взял из заморозки");
        assert_eq!(d.earned_at(8_000).unwrap(), 100);
    }
