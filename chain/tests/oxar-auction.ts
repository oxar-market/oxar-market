...
describe("oxar-escrow: торг", () => {
  ...
  it("жёсткий конец записывается при открытии: заявленный срок плюс час", async () => {
    // Дальше него ставки торг не утянут, сколько бы их ни было. Сама
    // арифметика упора проверена юнитами в sale.rs - здесь только то, что
    // поле действительно ложится в аккаунт.
    const { sale, closesAt } = await openSale(60);
    const state = await program.account.sale.fetch(sale);
    assert.equal(
      state.hardClosesAt.toNumber(),
      closesAt + 3_600,
      "потолок лёг не туда",
    );
  });
